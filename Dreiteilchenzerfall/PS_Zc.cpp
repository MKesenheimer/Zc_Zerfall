 ...
 * der Bindungsenergie Epsilon und des Größenparamters Lambda
 * zu berechnen
 */
...
#define mPi 0.1396
#define mJ 3.0969
#define mGam 0.002 //0.150 //Cutoff Parameter für die Photonmasse
#define mD 1.865
#define mDs 2.010
...
#define SEEDF 0
#define MINEVALF 0
#define MAXEVALF 1000000//1000000 //erst bei 2.5M Evaluationen wird eine relative Genauigkeit von 1e-2 erreicht! Mit 10000000 ist man sicher.
...
int main() {
	...
	for (int neps = STRTEPS; neps < EPSCOUNT; neps++) {
		for (int nlam = STRTLAM; nlam < LAMCOUNT; nlam++) {
                  ...
                  oFF << endl;
                  //Tabelle vorbereiten
                  oFF << "s12\ts23\ts31\t\tFF1\t\tFF23\t\tFF4\t\tFF5\t\tFF6" << endl;
                  oFF.close();
                  ...
                  dGamma << endl;
                  //Tabelle vorbereiten
                  dGamma << "s12\ts23\ts31\t\tdGamma" << endl;
                  dGamma.close();
#endif
			...
			Cuhre(NDIMPS, NCOMPPS, Phasenraum, dataps,
                 ...
                 cyclePS = (neps*2+nlam+1)*MAXEVALPS;
#endif
#if INTMETH == SUAVE
	...
	for (int comp = 0; comp < NCOMPPS; comp++) {
      ...
      //if ( a != 'n') {
            ...
            for (int comp = 0; comp < NCOMPPS; comp++) {
                  for (int neps = 0; neps < EPSCOUNT; neps++) {
                        for (int nlam = 0; nlam < LAMCOUNT; nlam++) {
                              if (nlam == 0) {
