 ...
 */
/* Kompilieren mit g++ -o dG_Zc dG_Zc.cpp -lcuba -lm -lstdc++ -lncurses -O3 */

/*
 * Programm um die doppelt differentielle Zerfallsrate dG/ds12ds34 in Abhängigkeit
...
#define m1 mPi
#define m2 mJ
#define m3 me //Berechne den Vierteilchenzerfall mit einem Elektron-Positron-Paar
#define m4 me
#define ml me
#define MD mD
#define MDs mDs
...
#define NBATCHF 1000
#define GRIDNOF 0 //
#define STATEFILEF NULL //"statefileFF.dat"
#define NNEWF 1000
#define FLATNESSF 25.
#define KEYF 0
#define KEY1F 47
...
#define GRIDNOPS 0
#define STATEFILEPS NULL //"statefilePS.dat"
#define NNEWPS 1000
...
double getPercent(void){
      percent = (float)(cyclePS)/(EPSCOUNT*LAMCOUNT*MAXNS*MAXNS/1)*100;
...
void getStatus(double s12, double s34, double gZc, double g, double eps,
               double lam, double F1, double errF1, double F23, double errF23, double F4, double errF4,
               double F5, double errF5, double F6, double errF6, int nregionsf, int nevalf, int failf, double dGam){
      clear();
      move(0,0);
      ...
      //Werte ausgeben
      printw("cyclePS:\t%d\ns12:\t\t%f\t[GeV^2]\ns34:\t\t%f\t[GeV^2]\ngZc:\t\t%f\ng:\t\t%f\nmGam:\t\t%.0f\t\t[MeV]\n\nF1:\t\t%f +- %f\nF23:\t\t%f +- %f\n",
             cyclePS, s12, s34, gZc, g, mGam*1000, F1, errF1, F23, errF23);
      printw("F4:\t\t%f +- %f\nF5:\t\t%f +- %f\nF6:\t\t%f +- %f\n\n",
             F4, errF4, F5, errF5, F6, errF6);

      printw("nregionsf: %d\t nevalf: %d\t failf: %d\n",nregionsf, nevalf, failf);
      printw("NaN Counter:\t\t%d\n",isnanFail);
      printw("dGam:\t\t%f\t[10^-6 GeV^-3]\n\n",dGam);
      //Ladebalken
      move(18,0);
      printw("[");
...
double s12p(double eps) {
      return (pow(M(eps)-m3-m4,2));
}

double s34p(double s12, double eps) {
      return (pow(M(eps)-sqrt(s12),2));
}
...
static int Formfaktoren(const int *ndim, const double xx[],
  const int *ncomp, double ff[], void *userdata) {
	...
	//Notiz: Aufpassen!
      ...
      ff5 = dm2*4*cc*exp(-r5_2/a5-M5_2)/(Power(a5,2)); //(Power(m1,2)-s134) //Der Propagator steht für diese Rechnung im Matrixelement
...
/*                                                                   */
/*********************************************************************/
double *F(double eps, double lam, double s12, double s34, double gZc) {
...
double Minv(double eps, double lam, double s12, double s34, double xsi, double tht, double phi, double gZc,
            double F1, double F23, double F4, double F5, double F6) {
      
      //Matrixelement laden
      double minv = getMinv(eps, lam, s12, s34, xsi, tht, phi, F1, F23, F4, F5, F6);
...
static int Phasenraum(const int *ndim, const double xx[],
	...
	double gZc = *((double*)userdata+2);
      double s12 = *((double*)userdata+3);
      double s34 = *((double*)userdata+4);
      double F1 = *((double*)userdata+5);
      double F23 = *((double*)userdata+6);
      double F4 = *((double*)userdata+7);
      double F5 = *((double*)userdata+8);
      double F6 = *((double*)userdata+9);
      ...
      double jac = 1/(s34*s12)*ksqrt(pow(M(eps),2),s12,s34)*ksqrt(s12,pow(m1,2),pow(m2,2))*ksqrt(s34,pow(m3,2),pow(m4,2))*dxsi*dtht*dphi*sin(tht)*sin(xsi);
      
      //Phasenraumintegrant mit angepassten Grenzen 0 bis 1
      dG = c*Minv(eps, lam, s12, s34, xsi, tht, phi, gZc, F1, F23, F4, F5, F6)*jac;
...
/*                                                                   */
/*********************************************************************/
void integrate_angles(double s12, double s34, double eps, double lam, double gZc) {
	...
	double integralps[NCOMPPS], errorps[NCOMPPS], probps[NCOMPPS];
      ...
      double *ptr = F(eps, lam, s12, s34, gZc);
      ...
      double dataps[] = {eps, lam, gZc, s12, s34, F1, F23, F4, F5, F6};
      ...
      #ifndef DEBUG
        #ifndef OPTIMIZED
          if( !isnan(integralps[0]) )
            getStatus(s12, s34, gZc, g(eps), eps, lam, F1, errF1, F23, errF23, F4, errF4,
                      F5, errF5, F6, errF6, nregionsf, nevalf, failf, integralps[0]*1000000);
        #endif
      #endif
      ...
      #ifndef OPTIMIZED
      if (s12 <= s12p(s34,eps) && s12 >= s12m) {
            string filename = "dGamma_eps";
...
int main() {
	...
	for (int neps = STRTEPS; neps < EPSCOUNT; neps++) {
		for (int nlam = STRTLAM; nlam < LAMCOUNT; nlam++) {
                  ...
                  dGamma << endl;
                  //Tabelle vorbereiten
                  dGamma << "s12\ts34\t\tdGamma" << endl;
                  dGamma.close();
#endif
                  
                  //Integrationsschleife für die Punkte (s12,s34)
                  for (int ns34 = 1; ns34 <= MAXNS; ns34++) {
                        int MAXNS12;
                        MAXNS12 = MAXNS - ns34;
            
                        for (int ns12 = 1; ns12 <= MAXNS12; ns12++) { //Den Phasenraum durch eine Gerade annähern, um die Punktedichte ungefähr konstant zu halten
                              
                              double q34 = (double)ns34/(double)MAXNS; //läuft von [~0,1]
                              double q12 = (double)ns12/(double)MAXNS12;
                              double s34 = s34m + (s34p(eps) - s34m)*q34; //Invariante Massen
                              double s12 = s12m + (s12p(s34,eps) - s12m)*q12;
                              
                              integrate_angles(s12, s34, eps, lam, gZc);
                        }
                  }
                  
                  for (int ns12 = 0; ns12 <= MAXNS; ns12++) {
                        int MAXNS34;
                        MAXNS34 = MAXNS - ns12;
                        
                        for (int ns34 = 0; ns34 <= MAXNS34; ns34++) { //Den Phasenraum durch eine Gerade annähern, um die Punktedichte ungefähr konstant zu halten
                              
                              double q12 = (double)ns12/(double)MAXNS; //läuft von [~0,1]
                              double q34 = (double)ns34/(double)MAXNS34;
                              double s12 = s12m + (s12p(eps) - s12m)*q12; //Invariante Massen
                              double s34 = s34m + (s34p(s12,eps) - s34m)*q34;
