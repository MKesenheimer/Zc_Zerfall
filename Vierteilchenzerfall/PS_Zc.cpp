...
#define STRTEPS (1-1)       //wo mit Rechnen anfangen?
#define STRTLAM (1-1)       //wo mit Rechnen anfangen?
...
//Optimierungen
//#define CUBACORES -2 //Benutze bis zu n CPU-Kern(e)
...
double getPercent(void){
      percent = (float)(cyclePS)/((EPSCOUNT-STRTEPS)*(LAMCOUNT-STRTLAM)*MAXEVALPS)*100;
...
void getStatus(double s12, double s34, double xsi, double tht, double phi, double gZc, double g, double eps,
      ...
      oFF.open(filename.c_str(), ios::out | ios::app);
      oFF << s12 << "\t" << s34 << "\t" << xsi << "\t" << tht << "\t" << phi << "\t" << F1 << "\t"
      << F23 << "\t" << F4 << "\t" << F5 << "\t" << F6 << endl;
      oFF.close();
}
...
double getgZc(int neps, int nlam) {
      ...
      */
      ...
      ifstream is ("gZc.data", ifstream::binary);
      if (is) {
            // get length of file:
            is.seekg (0, is.end);
            int length = is.tellg();
...
#ifdef OPTIMIZED
      if((cyclePS % (MAXEVALPS/50)) == 1)
            printf("%.0f\t%d\n",getPercent(),cyclePS-1);
#endif
...
static int Phasenraum(const int *ndim, const double xx[],
	...
	double gZc = *((double*)userdata+2);
     ...
     #define c (1.0/(3.0*pow(2.0,15.0)*pow(M(eps),3.0)*pow(M_PI,6.0)))
      ...
      //Umdefinition der Variaben [0, 1] -> [s12m(s34), s12p(s34)] und [0, 1] -> [s34m, s34p]:
      double s34 = s34m - q34*(s34m - s34p(eps));
      double ds34 = (s34p(eps) - s34m);
      ...
      double jac = 1/(s34*s12)*ksqrt(pow(M(eps),2),s12,s34)*ksqrt(s12,pow(m1,2),pow(m2,2))*ksqrt(s34,pow(m3,2),pow(m4,2))*ds12*ds34*dxsi*dtht*dphi*sin(tht)*sin(xsi);
...
int main() {
	...
	for (int neps = STRTEPS; neps < EPSCOUNT; neps++) {
		for (int nlam = STRTLAM; nlam < LAMCOUNT; nlam++) {
                  ...
                  filename.append(to_string((int)(lam*1000))); //Lambda in MeV
                  filename.append("MeV.dat");
                  oFF.open(filename.c_str(), ios::out); //alte Dateien löschen!
                  //Header
                  oFF << "eps: " << eps*1000 << "MeV" << endl;
                  oFF << "lam: " << lam*1000 << "MeV" << endl;
                  oFF << "mGam: " << mGam*1000 << "MeV" << endl;
                  oFF << "mLept: " << ml*1000 << "MeV" << endl;
                  oFF << "MAXEVALF: " << MAXEVALF << endl;
                  oFF << "MAXEVALPS: " << MAXEVALPS << endl;
                  oFF << endl;
                  //Tabelle vorbereiten
                  oFF << "s12\ts34\t\txsi\ttht\tphi\t\tFF1\t\tFF23\t\tFF4\t\tFF5\t\tFF6" << endl;
                  oFF.close();
#endif
			...
			Cuhre(NDIMPS, NCOMPPS, Phasenraum, dataps,
                 ...
                 cyclePS = (neps-STRTEPS)*2+(nlam-STRTLAM+1)*MAXEVALPS;
#endif
#if INTMETH == SUAVE
...
#endif
                  refresh();
                  for (int comp = 0; comp < NCOMPPS; comp++) {
				resultps[neps][nlam][comp] = integralps[comp];
				gerrorps[neps][nlam][comp] = errorps[comp];
			}
                  
                  //Zwischenergebnis in Datei schreiben
			//Dateiausgabe vorbereiten
                  #ifndef OPTIMIZED
                  filename = "result_eps";
                  #endif
                  #ifdef OPTIMIZED
                  string filename = "result_eps";
                  #endif
                  filename.append(to_string((int)(eps*1000000))); //Epsilon in keV an den Dateiname anhängen
                  filename.append("keV_lam");
                  filename.append(to_string((int)(lam*1000))); //Lambda in MeV
                  filename.append("MeV.dat");
                  results.open(filename.c_str(), ios::out); //alte Dateien löschen!
                  //Header
                  results << "eps: " << eps*1000 << "MeV" << endl;
                  results << "lam: " << lam*1000 << "MeV" << endl;
                  results << "mGam: " << mGam*1000 << "MeV" << endl;
                  results << "mLept: " << ml*1000 << "MeV" << endl;
                  results << "MAXEVALF: " << MAXEVALF << endl;
                  results << "MAXEVALPS: " << MAXEVALPS << endl;
                  results << endl;
                  //Ergebnis ausgeben
                  results << "Gamma =\t" << resultps[neps][nlam][0] * 1000 << " +- " << gerrorps[neps][nlam][0] * 1000 << endl;
                  results.close();
		}
	}
#ifndef OPTIMIZED
	...
	printw("\nRESULT:\tnregions %d\tneval %d\tfail %d\n", nregionsps, nevalps, failps);
      printw("\nAlle Grössen in [MeV].\n\n");
      printw("eps/lam\t\t");
      
      for (int nlam = 0; nlam < LAMCOUNT; nlam++) {
            printw("%.0f\t\t\t", (lambda + lambdaStep * nlam) * 1000);
      }
      printw("\n");
	...
	for (int comp = 0; comp < NCOMPPS; comp++) {
		for (int neps = STRTEPS; neps < EPSCOUNT; neps++) {
			for (int nlam = STRTLAM; nlam < LAMCOUNT; nlam++) {
                        if (nlam == 0) {
                              printw("%.1f\t", (epsilon + epsilonStep * neps) * 1000);
                        }
				printw("%f +- ",resultps[neps][nlam][comp] * 1000);
				printw("%.3f\t",gerrorps[neps][nlam][comp] * 1000);
			}
			printw("\n");
		}
		printw("\n");
      ...
      printw("Rechenzeit: %1d Sekunden.\n", (int)(end - start));
      printw("Rechenfehler (NaN): %d\n\n", isnanFail);
      refresh();
      
      //Ergebnisse in Datei schreiben
      //printw("Ergebnisse in Datei speichern und vorige Datei überschreiben? [y,n]\n");
      //refresh();
      //char a = getch();
      //if ( a != 'n') {
            string filename = "results";
            filename.append(to_string((int)(EPSCOUNT/2))); //Epsilon in keV an den Dateiname anhängen
            filename.append(".dat");
            results.open(filename.c_str(), ios::out); //alte Dateien löschen!
            ...
            results << "MAXEVALF: " << MAXEVALF << endl;
            results << "MAXEVALPS: " << MAXEVALPS << endl;
            results << "RESULT:\tnregions " << nregionsps << "\tneval " << nevalps << "\tfail " << failps << "\n";
            results << "\nAlle Grössen in [MeV].\n\n";
            results << "eps/lam\t\t";
      
            for (int nlam = STRTLAM; nlam < LAMCOUNT; nlam++) {
                  results << (lambda + lambdaStep * nlam) * 1000 << "\t\t\t";
            }
            results << "\n";
            ...
            for (int comp = 0; comp < NCOMPPS; comp++) {
                  for (int neps = STRTEPS; neps < EPSCOUNT; neps++) {
                        for (int nlam = STRTLAM; nlam < LAMCOUNT; nlam++) {
                              if (nlam == 0) {
                                    results << (epsilon + epsilonStep * neps) * 1000 << "\t";
                              }
                              results << resultps[neps][nlam][comp] * 1000 << " +- ";
                              results << gerrorps[neps][nlam][comp] * 1000 << "\t";
                        }
                        results << "\n";
                  }
                  results << "\n";
            ...
            results << "Rechenzeit: " << (int)(end - start) << " Sekunden.\n";
            results << "Rechenfehler (NaN): " << isnanFail << "\n\n";
            results.close();
